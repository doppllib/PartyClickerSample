...
+ (const J2ObjcClassInfo *)__metadata {
  ...
  static const J2ObjcFieldInfo fields[] = {
    ...
    { "window_", "LRXSSubject;", .constantValue.asLong = 0, 0x0, -1, -1, 8, -1 },
  };
...
+ (const J2ObjcClassInfo *)__metadata {
  ...
  static const J2ObjcClassInfo _RxInternalOperatorsOperatorWindowWithSize_WindowExact_1 = { "", "rx.internal.operators", ptrTable, methods, fields, 7, 0x8018, 2, 1, 3, -1, 4, -1, -1 };
...
- (void)onNextWithId:(id)t {
  ...
  if (i == size_) {
    index_ = i;
    JreStrongAssign(&window_, nil);
    [((RXSSubject *) nil_chk(w)) onCompleted];
  }
  else if (i == skip_) {
    index_ = 0;
  }
...
- (void)onNextWithId:(id)t {
  ...
  if (i == 0 && ![((RxSubscriber *) nil_chk(actual_)) isUnsubscribed]) {
    [((JavaUtilConcurrentAtomicAtomicInteger *) nil_chk(wip_)) getAndIncrement];
    RXSSubject *w = RXSUnicastSubject_createWithInt_withRxFunctionsAction0_(16, self);
    [((JavaUtilArrayDeque *) nil_chk(q)) offerWithId:w];
    [((id<JavaUtilQueue>) nil_chk(queue_)) offerWithId:w];
    [self drain];
  }
  for (RXSSubject * __strong w in nil_chk(windows_)) {
    [((RXSSubject *) nil_chk(w)) onNextWithId:t];
  }
  jint p = produced_ + 1;
  if (p == size_) {
    produced_ = p - skip_;
    RXSSubject *w = [((JavaUtilArrayDeque *) nil_chk(q)) poll];
    if (w != nil) {
      [w onCompleted];
    }
  }
...
- (void)onErrorWithNSException:(NSException *)e {
  for (RXSSubject * __strong w in nil_chk(windows_)) {
    [((RXSSubject *) nil_chk(w)) onErrorWithNSException:e];
  }
  [windows_ clear];
...
- (void)drain {
  ...
  for (; ; ) {
    ...
    while (e != r) {
      ...
      RXSSubject *v = [((id<JavaUtilQueue>) nil_chk(q)) poll];
