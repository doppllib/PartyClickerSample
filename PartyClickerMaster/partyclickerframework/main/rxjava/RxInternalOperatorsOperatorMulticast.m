...
- (void)connectWithRxFunctionsAction1:(id<RxFunctionsAction1>)connection {
  @synchronized(guard_) {
    if (subscription_ != nil) {
      ...
      RXSSubject *subject = [((id<RxFunctionsFunc0>) nil_chk(subjectFactory_)) call];
      ...
      for (RxSubscriber * __strong s in nil_chk(waitingForConnect_)) {
        [((RXSSubject *) nil_chk(subject)) unsafeSubscribeWithRxSubscriber:create_RxInternalOperatorsOperatorMulticast_3_initWithRxSubscriber_withRxSubscriber_(s, s)];
      }
...
- (void)callWithId:(RxSubscriber *)subscriber {
  @synchronized(val$guard_) {
    if ([((JavaUtilConcurrentAtomicAtomicReference *) nil_chk(val$connectedSubject_)) get] == nil) {
      ...
      [((RXSSubject *) nil_chk([val$connectedSubject_ get])) unsafeSubscribeWithRxSubscriber:subscriber];
    }
  }
}
